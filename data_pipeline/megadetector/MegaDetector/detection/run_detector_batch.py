...
def consumer_func(q, return_queue, model_file, confidence_threshold, image_size=None):
    ...
    print(
        'Loaded model (before queueing) in {}, printing updates every {} images'.format(
    ...
    while True:
        ...
        if verbose:
            print('Processed image {}'.format(im_file));
            sys.stdout.flush()
        q.task_done()
...
def process_image(detector, confidence_threshold, im_file=None, image=None,
...
def load_custom_class_mapping(class_mapping_filename):
    ...
    if class_mapping_filename.endswith('.json'):
    ...
    elif (class_mapping_filename.endswith('.yml') or class_mapping_filename.endswith(
            '.yaml')):
        from data_management.yolo_output_to_md_output import \
        ...
        raise ValueError(
            'Unrecognized class mapping file {}'.format(class_mapping_filename))
...
def load_and_run_detector_batch(model_file, images=None, image_file_names=None, checkpoint_path=None,
    ...
    if isinstance(image_file_names, str):
        ...
        if os.path.isdir(image_file_names):
            ...
            print('{} image files found in folder {}'.format(len(image_file_names),
                                                             image_dir))
        ...
        # A single file, or a list of image paths
            ...
            if image_file_names.endswith('.json'):
                ...
                with open(list_file, 'r') as f:
                    ...
                    image_file_names = [s.strip() for s in image_file_names if
                                        len(s.strip()) > 0]
    ...
    if n_cores > 1 and is_gpu_available(model_file):
        print(
            'Warning: multiple cores requested, but a GPU is available; parallelization across ' + \
            'GPUs is not currently supported, defaulting to one GPU')
        n_cores = 1

    if n_cores > 1 and use_image_queue:
        print(
            'Warning: multiple cores requested, but the image queue is enabled; parallelization ' + \
    ...
    if use_image_queue:
        ...
        if image_file_names:
            for im_file in tqdm(image_file_names):  # im_file: filepath
        ...
        if len(already_processed) > 0:
            ...
            image_file_names = [fn for fn in image_file_names if
                                fn not in already_processed]
...
def checkpoint_queue_handler(checkpoint_path, checkpoint_frequency, checkpoint_queue,
                             results):
    """
...
def write_results_to_file(results, output_file, relative_path_base=None,
    ...
    if info is None:
        ...
        if detector_file is not None:
            ...
            detector_metadata = get_detector_metadata_from_version_string(
                detector_version)
            info['detector'] = detector_filename
            info['detector_metadata'] = detector_metadata
        else:
            info['detector'] = 'unknown'
            info['detector_metadata'] = get_detector_metadata_from_version_string(
                'unknown')
...
def batch_detection(args, images=None):
    ...
    if args.output_file:
        if os.path.exists(args.output_file):
            if args.overwrite_handling == 'overwrite':
                print(
                    'Warning: output file {} already exists and will be overwritten'.format(
                        args.output_file))
                ...
                raise ValueError('Illegal overwrite handling string {}'.format(
                    args.overwrite_handling))
    ...
    # Find the images to score; images can be a directory, may need to recurse
    if args.image_file:  # cases where image filenames are given rather than the images themselves
        if os.path.isdir(args.image_file):
            ...
            if len(image_file_names) > 0:
                ...
                if args.recursive:
                    print('No image files found in directory {}, exiting'.format(
                        args.image_file))
                else:
        ...
        # A text list of image paths
            ...
            with open(args.image_file) as f:
                image_file_names = f.readlines()
                image_file_names = [fn.strip() for fn in image_file_names if
                                    len(fn.strip()) > 0]
            print('Loaded {} image filenames from .txt list file {}'.format(
                len(image_file_names), args.image_file))
        ...
        assert len(
            image_file_names) > 0, 'Specified image_file does not point to valid image files'
        assert os.path.exists(image_file_names[0]), \
            'The first image to be processed does not exist at {}'.format(
                image_file_names[0])
    ...
    if args.checkpoint_frequency != -1:
        ...
        if args.checkpoint_path is not None:
            ...
            checkpoint_path = os.path.join('checkpoint_{}.json'
    ...
    results = load_and_run_detector_batch(model_file=args.detector_file,
                                          image_file_names=image_file_names if args.image_file else None,
                                          images=images if not args.image_file else None,
